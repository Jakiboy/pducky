...
class BinaryInstaller
{
    ...
    private const BIN_DIR = __DIR__ . '/../src/bin';
    ...
    {
        $winPath = self::BIN_DIR . '/win/duckdb.exe';
        $linPath = self::BIN_DIR . '/lin/duckdb';
    ...
    {
        $linuxBinary = self::BIN_DIR . '/lin/duckdb';
        if (file_exists($linuxBinary)) {
            chmod($linuxBinary, 0755);
        }
        
        $linuxLib = self::BIN_DIR . '/lin/libduckdb.so';
    ...
    private function cleanup(string $zipPath): void
    {
        ...
        if (is_dir(self::TEMP_DIR) && count(scandir(self::TEMP_DIR)) === 2) {
            rmdir(self::TEMP_DIR);
        }
    }
